<?php

$servername = "localhost";
$username = "root";
$password = "mysql";
$dbname = "ivi";

$conn = new mysqli($servername, $username, $password, $dbname);


if ($conn->connect_error) {
    die("Connection failed: " . $conn->connect_error);
}


$sql = "SELECT id_series FROM series WHERE id_series > 36";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    
    $person_ids = array();
    $sql = "SELECT id_person FROM person";
    $person_result = $conn->query($sql);
    if ($person_result->num_rows > 0) {
        while ($row = $person_result->fetch_assoc()) {
            $person_ids[] = $row['id_person'];
        }
    }

    
    while ($row = $result->fetch_assoc()) {
        $series_id = $row['id_series'];

        
        $num_roles = mt_rand(1, 5);

        
        $selected_persons = array_rand($person_ids, $num_roles);

        
        foreach ($selected_persons as $person_index) {
            $person_id = $person_ids[$person_index];
            $role_name = mt_rand(0, 1) ? 'Актер' : 'Режиссер'; // Случайный выбор роли
            $sql = "INSERT INTO role_person (name_role, id_person, id_series) VALUES ('$role_name', '$person_id', '$series_id')";
            if ($conn->query($sql) === FALSE) {
                echo "Error: " . $sql . "<br>" . $conn->error;
            }
        }
    }
} else {
    echo "No series found with id > 36";
}

$conn->close();
?>